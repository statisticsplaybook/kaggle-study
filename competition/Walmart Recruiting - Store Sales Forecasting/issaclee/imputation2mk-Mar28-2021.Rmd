...
train %<>%
  ...
  # mutate(weekly_sales = sign(weekly_sales) * log(abs(weekly_sales) + 2))
...
train2_isholiday <- train2 %>% filter(is_holiday == FALSE)
...
# validation_split <- validation_split(train2_isholiday,
#                                      strata = weekly_sales)
```
...
result %<>%
  ...
  # mutate(.pred = sign(.pred) * (abs(exp(.pred)) - 2))
```
