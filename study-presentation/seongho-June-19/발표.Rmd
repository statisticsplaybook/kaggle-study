...
train_base_dinner<-train_base_dinner %>% mutate(holiday= factor(holiday))
...
날씨데이터에 대해서 식자재 발주를 고려한다면 초단기 예측 데이터를 사용할 수 없냐고 여쭤보니
가능하다고 합니다. 
재택근무비율이 생각보다 영향이 미미한데, 2016~2019년치데이터에서는 없었던 변수나 다름 없고,
2020년 1년 그것도 몇개월 없는 데이터여서 그런 것 같습니다. 아니면 기숙사 생활을 많이 해서??
