...
I used recipe because I want to keep my notebook structure even if we use mean model. The following recipe is just simple like this; pull `month` from `date` variable and remove `date` variable. Done.
...
```{r}
train2 %<>% add_column(result_train)

train2 %>% head()

train2 %>% select(weekly_sales, result_train) %>% plot()
```
